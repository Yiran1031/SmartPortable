    ...
    for(var i=0; i<zipCodeArr.length; i++) {
        ...
        for(var j=0; j<productNameArr.length; j++) {
            for(k=0; k<parsedData.length; k++) {
                if(parsedData[k]["num"] === zipCodeArr[i] && parsedData[k]["name"] === productNameArr[j]&& parsedData[k]["name"] === productNameArr[i]) {
                    dataArr[j+1] = parseInt(parsedData[k]["num"]);
                }
            }
