...
function createDataTable(jsonData) {
    ...
    for(var i=0; i<parsedData.length; i++) {
        var productName = parsedData[i]["orderName"];
        var zipCode = parsedData[i]["count"];
    ...
    for(var i=0; i<zipCodeArr.length; i++) {
        ...
        for(var j=0; j<productNameArr.length; j++) {
            for(k=0; k<parsedData.length; k++) {
                if(parsedData[k]["count"] === zipCodeArr[i] && parsedData[k]["orderName"] === productNameArr[j]&& parsedData[k]["orderName"] === productNameArr[i]) {
                    dataArr[j+1] = parseInt(parsedData[k]["count"]*parsedData[k]["price"]);
                }
            }
