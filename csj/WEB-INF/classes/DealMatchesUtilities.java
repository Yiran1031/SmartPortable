...
public class DealMatchesUtilities extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter pw = response.getWriter();
			...
			HashMap<String,ProductDetail> selectedproducts=new HashMap<String,ProductDetail>();
		try
			{
		...
		pw.print("<a href='#'>Welcome to SmartPortable </a></h2>");
		...
		pw.print("<h1>We beat our competitors in all aspects. Price-Match Guaranteed</h2>");
			...
			HashMap<String,ProductDetail> productmap=MySqlDataStoreUtilities.getData();
			
			for(Map.Entry<String, ProductDetail> entry : productmap.entrySet())
			{
...
//		
		...
		pw.print("<tr>");
		for(Map.Entry<String, ProductDetail
			> entry : selectedproducts.entrySet()){
		pw.print("<td><div id='shop_item'><h3>"+entry.getValue().getName()+"</h3>");
		pw.print("<strong>"+entry.getValue().getPrice()+"$</strong>");
		pw.print("<ul>");
		pw.print("<li id='item'><img src='images/"+entry.getValue().getImage()+"' alt='' />");
		pw.print("</li><li>");
		...
		pw.print("<input type='hidden' name='product' value="+entry.getValue().getType()+">");
		...
		pw.print("<input type='hidden' name='name' value='"+entry.getValue().getId()+"'>");
		pw.print("<input type='hidden' name='type' value='"+entry.getValue().getType()+"'>");
		pw.print("<input type='hidden' name='maker' value='SmartPortable'>");
